...
from app.database.db import add_product, get_all_products, delete_product
import os
...
async def admin_add(callback, state: FSMContext):
    ...
    await callback.message.edit_text("Введи штрихкод товара:")
...
async def add_status(callback, state: FSMContext):
    status = callback.data.replace("status_", "")
    data = await state.get_data()
    await state.clear()
    await add_product(data["barcode"], data["name"], data["brand"], status)
    await callback.message.edit_text(f"✅ Товар добавлен!\n\n<b>{data['name']}</b>\nШтрихкод: {data['barcode']}\nСтатус: {status}", parse_mode="HTML")
...
async def admin_list(callback):
    ...
    for p in products[:20]:
        status_emoji = {"halal": "✅", "haram": "❌", "doubtful": "⚠️"}.get(p["status"], "❓")
        text += f"{status_emoji} <b>{p['name']}</b> — {p['barcode']}\n"
